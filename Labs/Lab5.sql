...
--9
select contactfirstName, contactlastName, creditLimit from customers where creditLimit = 0;
--10
select * from offices where country != 'USA';
--11
select * from orders where orderDate>= '2014-06-16' and orderdate <= '2014-07-07';
--12
...
-- NATURAL JOIN -- Implicit join clause based on the common values, not specific
--18
...
--20
select customerName from customers inner join offices using (state);
--21
select customerName from customers inner join employees on (salesrepemployeenumber = employeenumber) inner join offices using (state);
...
where (shippedDate >= '2015-01-01' and shippedDate <= '2015-12-31') 
and (orderDate >= '2015-01-01' and orderDate <= '2015-12-31');

-- OUTER JOIN
--23
select productName from products left outer join orderdetails using (productcode) where ordernumber is null;
--24
select customername, salesrepemployeenumber from customers left outer join employees on(salesrepemployeenumber = employeenumber);
...
--31
select officecode, count(employeenumber) as NumberofEmployees from offices inner join employees using (officecode) group by (officecode);

-- HAVING
--32
select productline as "Productline", count(productcode) as "Number of products" from products 
group by productline having count(productcode)>3;
--33 not needed

-- COMPUTATIONS
--34 not needed
--35 Not sure about the syntax for sum
select productcode,productname, sum(msrp-buyprice) as Profit from products 
...
--1
select customerName, productName, count(orderNumber) as "NUMBER OF TIMES ORDERED"
from customers inner join orders natural join orderDetails natural join products
...
--2
select productName, sum(msrp / buyPrice * 100) as "PERCENTAGE PROFIT" 
from products 
group by productName
having sum(msrp / buyPrice * 100) >= 230
order by sum(msrp / buyPrice * 100) DESC;
...
--4
select orderNumber, orderDate, customerName, count(productCode) as "NUMBER OF SEPARATE PRODUCTS ORDERED"
